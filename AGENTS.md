...
**Üçüncü katman — `gh pr create`/`gh pr edit` (12.09.2026'da eklendi):** Yukarıdaki iki kanca yalnızca **commit mesajlarını** görür; bir PR açıklaması commit mesajı değildir, bu yüzden `git` kancaları PR body'sine hiç bakamaz. 12.09.2026'da tam olarak bu boşluktan bir PR (#4), oturum linki body'sinde açıldı ve elle temizlenmek zorunda kaldı. Kapatma, `git` seviyesinde değil **Claude Code harness seviyesinde**: proje `.claude/settings.json` içinde bir `PreToolUse` kancası, her `Bash` tool-call'ının komut metnini tarar ve eşleşirse çağrıyı **reddeder** (commit-msg'nin sessizce silme davranışı değil, pre-push'un reddetme davranışı — bir shell komutunu güvenli biçimde yeniden yazmaya çalışmak yerine engellemek daha basit ve güvenlidir). Bu üçüncü katman `git` kancası **değildir** ve `core.hooksPath`'ten bağımsızdır; `.claude/settings.json` depoya commit edildiği için klonla birlikte gelir ama yalnızca Claude Code oturumlarını kapsar — `gh` CLI'nin doğrudan terminalden çağrılmasını engellemez.

Kapsam (12.09.2026, güvenlik incelemesi sonrası genişletildi): `gh pr create`/`gh pr edit`, `gh api .../pulls` ve GitHub'ın PR API'sine doğrudan `curl` ile (`api.github.com/.../pulls`) yapılan çağrılar — hepsi case-insensitive taranır. Desen eşleşmesi, ifadeyi salt açıklama olarak taşıyan komutları (ör. bu satırların kendisini içeren bir `git commit`) yanlışlıkla engellememek için **anchor'lı**: yalnızca komutun başında ya da `&&`/`;`/`|`/`$(` sonrasında aranır.

**Bilinen sınır (bilerek kayıtlı, kapatılmadı):** Bu kanca ham komut metnini regex ile tarar; gerçek bir shell parser değildir. Tırnaklama/IFS oyunlarıyla (`gh 'pr' 'create'`), değişken/alias/`eval` üzerinden dolaylı çağrıyla (`C="gh pr create"; $C ...`) ya da `hub` gibi alternatif bir CLI ile atlatılabilir. Bu, projenin kendi kabul ettiği bir risk: tehdit modeli kasıtlı bir atlatma girişimine karşı değil, Claude Code'un kendi doğal çağrı biçiminde kazara oturum linki sızdırmasına karşı bir emniyet ağıdır — `pre-push`'un kendi `--no-verify` sınırını nasıl belgelediğine benzer şekilde, kapatmaya çalışmak yerine burada belgeleniyor.
